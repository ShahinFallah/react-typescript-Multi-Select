...
function Select({ multiple, value, options, onChange }: SelectProps) {
  ...
  return (
    ...
    >
      <span
        className={`grow cursor-pointer flex gap-1 ${multiple ? "flex-wrap" : "truncate"}`}
      >
        {multiple
          ? value.map((o) => (
              <button
      ...
      >
        <IoIosClose className="text-2xl hover:scale-110 transition hover:text-red-300" />
      </button>
      ...
      <ul
        className={`transition-opacity ${isOpen ? "opacity-100 !visible" : "opacity-0"} invisible flex flex-col gap-2 bg-gradient-to-b from-slate-800 to-slate-700 absolute w-full top-[calc(100%+0.13rem)] left-0 rounded-lg p-2 max-h-64 overflow-y-auto custom-scrollbar`}
      >
        {options.map((o, index) => (
          <li
          ...
          >
            {o.label}
            {isOptionSelected(o) && (
              <span className="leading-none text-xl -translate-y-0.5 group-hover:text-red-500 group-hover:scale-125">
                ×
              </span>
            )}
          </li>
